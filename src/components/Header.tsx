...
export const Header: React.FC<HeaderProps> = ({
  ...
  const themeClasses = getThemeClasses(surauInfo.theme);

  return (
    <header className={`${themeClasses.headerBg} text-white shadow-xl border-b ${themeClasses.headerBorder} relative transition-colors duration-300`}>
      {/* Top Utility Bar */}
      <div className={`max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-2.5 border-b ${themeClasses.headerBorderSubtle} flex flex-wrap justify-between items-center text-xs ${themeClasses.headerSubtext} gap-2`}>
        <div className="flex items-center gap-4 flex-wrap">
          <span className="flex items-center gap-1.5 font-medium">
            <Landmark className={`w-3.5 h-3.5 ${themeClasses.headerIcon}`} />
            No. Daftar: <span className="text-white">{surauInfo.registrationNo}</span>
          </span>
          <span className="hidden md:inline opacity-40">•</span>
          <span className="hidden md:flex items-center gap-1.5">
            <span className={`${themeClasses.headerIcon} font-semibold`}>Akaun Bank:</span> {surauInfo.bankName} ({surauInfo.accountNo})
          </span>
        </div>

        <div className="flex items-center gap-2.5">
          <span className={`hidden sm:flex items-center gap-1.5 ${themeClasses.headerBadgeBg} ${themeClasses.headerBadgeText} font-medium px-2.5 py-1 rounded-md border ${themeClasses.headerBadgeBorder} shadow-sm`} title="Data tersimpan & disegerakkan secara automatik menerusi Cloud Firestore antara Telefon & PC">
            <Cloud className={`w-3.5 h-3.5 ${themeClasses.headerIcon} animate-pulse`} />
            <span className="text-[11px] font-bold">Awan Senkron (Real-Time)</span>
          </span>

          <span className={`hidden sm:flex items-center gap-1 ${themeClasses.headerBadgeBg} ${themeClasses.headerSubtext} px-2.5 py-1 rounded-md border ${themeClasses.headerBadgeBorder}`}>
          ...
          {isAdmin ? (
            <div className="flex items-center gap-1.5">
              <span className={`flex items-center gap-1 ${themeClasses.headerBadgeBg} text-amber-300 font-extrabold px-2.5 py-1 rounded-md border ${themeClasses.headerBadgeBorder} shadow-sm text-xs`}>
            ...
            <button
              ...
              title="Ketuk nama surau 5x atau tekan di sini untuk log masuk Pentadbir"
              className={`flex items-center gap-1 px-2.5 py-1 ${themeClasses.headerBadgeBg} ${themeClasses.headerSubtext} rounded text-xs font-medium border ${themeClasses.headerBadgeBorder} transition cursor-pointer`}
            >
          ...
          <div className="flex items-center gap-1.5">
            <button
              ...
              className={`flex items-center gap-1.5 px-3 py-1 ${themeClasses.headerBtn} rounded transition font-bold cursor-pointer border shadow-sm`}
            >
            ...
            <button
              ...
              title="Tetapan Surau"
              className={`flex items-center gap-1 px-3 py-1 ${themeClasses.headerBtnSecondary} rounded transition font-semibold cursor-pointer border`}
            >
              <Settings className={`w-3.5 h-3.5 ${themeClasses.headerIcon}`} />
              <span>Tetapan</span>
            </button>
          </div>
        </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-5 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="flex items-center gap-3.5">
          <div className="relative">
            ...
            <p className={`text-xs ${themeClasses.headerSubtext} line-clamp-1 max-w-xl mt-0.5`}>
              {surauInfo.address}
            </p>
          </div>
        </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <nav className={`flex space-x-2 sm:space-x-4 border-t ${themeClasses.headerBorderSubtle} pt-2 pb-0`}>
          <button
